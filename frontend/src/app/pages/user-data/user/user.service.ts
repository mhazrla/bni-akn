...
function matches(user: UserRole, term: string, pipe: PipeTransform) {
  if (!user) {
    return false;
  }
  ...
  return (
    ...
    role.includes(term.toLowerCase())
  );
}
...
export class UserService {
  ...
  private _countries$ = new BehaviorSubject<UserRole[]>([]);
  ...
  constructor(
    ...
    public restApiService: RestApiUserDataService,
    ...
    // Api Data
    this.restApiService.getAllUsers().subscribe((user) => {
      this.datas = user.data;
    });
  }
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 2. filter
    countries = countries.filter((user) =>
      matches(user, searchTerm, this.pipe)
    );
